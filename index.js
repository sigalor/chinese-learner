...
app.get('/strokeorder/:chars', async function (req, res) {
  ...
  await Promise.all(
    req.params.chars.split('').map(async (currChar, idx) => {
      //from https://stackoverflow.com/a/37576787
      ...
      let currRes = (
        await axios.post(
          'http://www.eon.com.hk/common/fcg/estroke.fcg?task=getPhrase',
  ...
  if (buildCache) fs.writeFileSync(charCacheFile, JSON.stringify(charCache));
