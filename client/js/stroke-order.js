function drawCharacterGridSVG(svg, gridType) {
	let w = 200, h = 200;		//all relative to viewbox
	let w3 = Math.round(w/3), h3 = Math.round(h/3);
	...
	switch(gridType) {
		...
		case "foursquare": allPaths = allPaths.concat([
				newPath(`M${w/2},0 L${w/2},${h}`),
				newPath(`M0,${h/2} L${w},${h/2}`)
			]);
			break;
		
		case "ninesquare": allPaths = [
...
function pointsToPath(points) {
	...
	for(let i = 1; i <= points.length; i++) {
		...
		if(points[currPointIdx].q)
			path += ` L${pointToStr(currPointIdx)}`;
		else {
			if(points[nextPointIdx].q) {
				...
				let targetPoint = {x: (points[currPointIdx].x+points[nextPointIdx].x)/2, y: (points[currPointIdx].y+points[nextPointIdx].y)/2};
...
function createCharOverlay() {
	...
	let currTimeout;
	let playButton = divClass("reload-button").append(playButtonIcon).click(async function() {
...
function getStrokeOrder(chars, charCacheFile, useCacheOnly) {
	return new Promise((resolve, reject) => {
		if(!charCacheFile  &&  useCacheFile) {
			console.warn(`ignored all characters ${chars}, because no character cache file is specified and only cache is allowed`);
			resolve([]);
			return;
		}
		
		if(charCacheFile) {
			$.getJSON(charCacheFile).then(cacheRes => {
				let ret = [], missingChars = [];
	...
	return new Promise((resolve, reject) => {
		getStrokeOrder(chars, charCacheFile, useCacheOnly).then(res => {
			...
			res.forEach(character => {
				let strokeOrderSVGContainerOuter = divClass("char-container-outer").css("width", `${sizeVW}vw`).css("height", `${sizeVW}vw`);
				...
				character.strokes.forEach((strokeGroup, idx) => {
					strokeGroup.forEach(stroke => {
						let pathElmt = $("<path/>").attr("d", pointsToPath(stroke.points, size))
							.addClass(["dir-left-right", "dir-top-bottom", "dir-right-left", "dir-bottom-top"][["leftToRight", "topToBottom", "rightToLeft", "bottomToTop"].indexOf(stroke.direction)])
					...
					if(idx != character.strokes.length - 1)
						currAnimDelay += 200;
				});
