...
function createMultipleChoice(options, correctIdx) {
  ...
  options.forEach((o, idx) => {
    mc.append(
      divClass('item')
        .append(divClass('item-container').append(divClass('item-content').html(o)))
        .attr('data-item-index', idx)
        .attr('data-is-correct', idx == correctIdx)
        .click(async function () {
          if ($(this).parent().hasClass('is-decided')) return;
...
function createKeyboard(keys, correctAnswer) {
  let kb = divClass('keyboard').append(
    divClass('input')
      .append(
        tagClass('input', 'text-input')
          .attr('type', 'text')
          .keyup(function (e) {
      ...
      .append(
        divClass('erase-char')
          ...
          .click(function () {
            ...
            if (kbParent.hasClass('is-decided')) return;
      ...
      .append(
        divClass('submit')
          .append(divClass('submit-content').append(faIcon('fa-angle-right')))
          .click(function () {
            let kbParent = $(this).parent().parent();
            if (kbParent.hasClass('is-decided')) return;
            let textInput = $(this).parent().children('input.text-input');
            //if(textInput.val() != "") {
            textInput
              .addClass(textInput.val().toLowerCase() == correctAnswer ? 'correct' : 'incorrect')
              .attr('readonly', 'true');
      ...
      divClass('key')
        ...
        .click(function () {
          if (!$(this).parent().parent().hasClass('is-decided')) {
            ...
            if (textInput.val().toLowerCase() == correctAnswer) $(this).parent().parent().find('.submit').click();
          }
        }),
    );
  });
...
function createWordChooser(options, correctAnswer) {
  let wc = divClass('word-chooser').append(
    divClass('input')
      .append(
        divClass('input-words').append(
          divClass('word dummy').append(divClass('word-container').append(divClass('word-content').html('&nbsp;'))),
        ),
      )
      .append(
        divClass('submit')
          ...
          .click(function () {
            $(this)
              .parent()
              .addClass(getCurrAnswer() == correctAnswer ? 'correct' : 'incorrect');
  ...
  let getCurrAnswer = () =>
    wc
      .find('.input > .input-words > .word:not(.dummy) > .word-container > .word-content')
      .toArray()
      .map(a => a.innerHTML)
      .join(' ');
  ...
  options.forEach((option, idx) => {
    optionsElmt.append(
      divClass('word')
        ...
        .click(function () {
          if ($(this).hasClass('chosen') || wc.hasClass('is-decided')) return;
          let inputElmt = $(this).parent().parent().children('.input');
          inputElmt.children('.input-words').append(
            $(this)
              .clone()
              .click(function () {
                if (wc.hasClass('is-decided')) return;
                let currIdx = parseInt($(this).attr('data-option-index'));
                $(this)
                  .parent()
                  .parent()
                  .parent()
                  .find(`.options > .word[data-option-index=${currIdx}]`)
                  .removeClass('chosen');
                $(this).detach();
              }),
          );
          ...
          if (getCurrAnswer() == correctAnswer) inputElmt.children('.submit').click();
        }),
    );
  });
...
function createTask(fromLang, toLang, wordObj) {
  let inputMethod,
    currAnswer = wordObj[toLang[0]];
  let taskParams, taskCorrectAnswer;

  if (fromLang[0] == 'german') {
    switch (toLang[0]) {
      case 'mandarin':
        inputMethod = currAnswer.length == 1 ? 'multiple-choice' : 'keyboard';
        break;
      case 'pinyin':
    ...
    switch (toLang[0]) {
      case 'german':
        inputMethod = currAnswer.length > 10 || currAnswer.indexOf(' ') != -1 ? 'word-chooser' : 'keyboard';
        break;
      case 'pinyin':
        inputMethod = 'keyboard';
        break;
  ...
  switch (inputMethod) {
    case 'multiple-choice': {
      ...
      taskParams = shuffleArrayCopy(
        vocabulary[currLectionIndex].words.map(w => w[toLang[0]]).filter(x => x != currAnswer),
      ).slice(0, 5);
    ...
    case 'keyboard': {
      ...
      taskParams = shuffleArray(
        currAnswer
          .split('')
          .concat(
            diffArray(shuffleArrayCopy(allLetters[currLectionIndex][toLang[0]]), currAnswer.split('')).slice(
              0,
              11 - currAnswer.length,
            ),
          ),
      );
    ...
    case 'word-chooser': {
      ...
      taskParams = shuffleArray(
        currAnswer
          ...
          .concat(
            diffArray(
              [].concat(...vocabulary[currLectionIndex].words.map(w => w[toLang[0]].split(' '))),
              currAnswer.split(' '),
            ).slice(0, 8 - currAnswer.split(' ').length),
          ),
      );
...
function createNextButton() {
  return divClass('next-button')
    .append(divClass('next-button-content').append(faIcon('fa-angle-right')))
    .click(async function () {
...
function initLearner(lectionIndex) {
  ...
  while (wordObj == lastWordObj) wordObj = getRandomEntry(vocabulary[lectionIndex].words);
  lastWordObj = wordObj;
...
async function buildCharCache(index) {
  for (let lection = index || 0; lection <= index || vocabulary.length - 1; lection++) {
    if (!confirm(`Now building cache for lection ${vocabulary[lection].name}.`)) continue;
