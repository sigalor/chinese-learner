...
function parseStrokeVector(strokeVector) {
	...
	for(let i = 2; i < decoded.length;) {
		...
		for(let j = 0; j < numStrokePoints; j++) {
			...
			//g := x, l := y, decoded.charAt(c) == "1" := q
			if(currX < currStroke.minX)
				currStroke.minX = currX;
			else if(currX > currStroke.maxX)
				currStroke.maxX = currX;
			if(currY < currStroke.minY)
				currStroke.minY = currY;
			else if(currY > currStroke.maxY)
				currStroke.maxY = currY;
			currStroke.strokePoints.push({ x: currX, y: currY, q: decoded.charAt(i) == "1" });
			i++;
		}

		ret.strokes.push(currStroke);
	}
...
function parseStrokeIdentifier(strokeIdentifier) {
	...
	for(let i = 0; i < strokeIdentifier.length; i += 2)
		ret.push(parseInt(strokeIdentifier.substr(i, 2), 16) & 255);
	return ret;
}
